...
<div>
	<div id="title">
		<div class="title left">
			<fmt:message key="global.system.title" />
		</div>
		<div style="float: right;">
			<span class="subtitle right"><fmt:message
					key="global.system.hello" />, ${loninName} <shiro:principal
					property="name" />!!【 <select id="locale" name="locale"
				onchange="changeLang(this.value)">
					<option value="zh_CN">
						<fmt:message key="global.system.lang.zh_CN" />
					</option>
					<option value="en">
						<fmt:message key="global.system.lang.en" />
					</option>
			</select> <a href="${ctx}/logout.do"><fmt:message
						key="global.system.logout" /> </a> 】 </span>
		</div>
	</div>
</div>
