...
<div>
	<c:choose>
		<c:when
				test="${shiroLoginFailure eq 'com.micmiu.mvc.hazel.core.action.exception.CaptchaException'}">
	...
	<form:form id="loginForm" action="${ctx}/login.do" method="post">
		<fieldset class="prepend-top">
			...
			<div class="field">
				<label for="captcha" class="field">验证码:</label>
				<input type="text" id="captcha" name="captcha" size="4" maxlength="4" class="required" />
			</div>
			<div class="field">
				<label for="img_captcha" class="field"></label>
				<img title="点击更换" id="img_captcha" onclick="javascript:refreshCaptcha();" src="servlet/captchaCode">(看不清<a href="javascript:void(0)" onclick="javascript:refreshCaptcha()">换一张</a>)
			</div>
		</fieldset>
		<div>
